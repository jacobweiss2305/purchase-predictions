...
def randomForestClassifier(X_train: pd.DataFrame, X_test: pd.DataFrame, y_train: pd.DataFrame, y_test: pd.DataFrame, n_feat: int):
    """Random Forest Classification model with recursive feature elimination.
    ...
    sfm = RFE(model, n_features_to_select=n_feat)
...
def adaBoostClassifier(X_train: pd.DataFrame, X_test: pd.DataFrame, y_train: pd.DataFrame, y_test: pd.DataFrame, n_feat: int):
    """Ada Boost Classification model with recursive feature elimination.
...
def xgboostClassifier(X_train: pd.DataFrame, X_test: pd.DataFrame, y_train: pd.DataFrame, y_test: pd.DataFrame, n_feat: int):
    ...
    model.fit(X_important_train, y_train)
    y_pred = model.predict(X_important_test[feature_list])
    print(f"Important Features: {feature_list}")
...
def gradientBoostingRegressor(X_train: pd.DataFrame, X_test: pd.DataFrame, y_train: pd.DataFrame, y_test: pd.DataFrame, n_feat: int) -> Any:
    ...
    metrics = {"model": "Gradient Boost", "metrics": {
        "mse": mse, "explained variance": var}}
